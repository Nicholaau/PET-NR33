(() => {
  ...
  async function createUser() {
    ...
    if (!body.name || !body.matricula || !body.password) return alert('Informe nome, matrícula e senha inicial.');
  ...
  async function refreshUsers() {
    ...
    try {
      ...
      box.innerHTML = data.users.map(u => `<div class="record-item"><div><strong>${escapeHtml(u.name)}</strong><br>Matrícula: ${escapeHtml(u.matricula)} • Perfil: ${escapeHtml(u.role)} • Status: ${escapeHtml(u.status)}<br><small>${escapeHtml(u.email || '')}</small></div></div>`).join('') || '<p class="hint">Nenhum usuário encontrado.</p>';
    } catch (err) {
  ...
  /** Aprova ou revoga chave de dispositivo. */
  async function changeDeviceStatus(id, action) {
    await apiFetch(`/devices/${encodeURIComponent(id)}/${action}`, { method: 'POST', body: {} });
  ...
  function bindEvents() {
    ...
    $('#registerDeviceBtn')?.addEventListener('click', () => registerDevice().catch(err => alert('Erro ao registrar dispositivo: ' + err.message)));
